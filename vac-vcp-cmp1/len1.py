# coding=utf-8
""" vac-vcp-cmp1/len1.py  Feb 21, 2014
...
class Vac(object):
 baseregex = r'(^.*?\t[0-9]+\t)|([ .+!_,*%-]+)|(<page>.*?</page>)|(<column>.*?</column>)|(<.*?>)|(\([0-9]+\))'
 regex = re.compile(baseregex)
 regex1 = re.compile("^%s$" % baseregex)
...
def len1(filein1,filein2,filecmp,fileout):
 ...
 for cmprec in cmprecs:
  update_chunks(cmprec.chunks1,recs1,Vac.regex,Vac.regex1)
  ...
  update_chunks(cmprec.chunks2,recs2,Vcp.regex,Vcp.regex1)
 ...
 print "# of these missing vac or vcp data = ",nmiss
 print "# of these with identical squashlines = ",nsame
 fout.close()
 
#-----------------------------------------------------
if __name__=="__main__":
 filein1 =sys.argv[1]  # vac
 filein2 =sys.argv[2]  # vcp
 filecmp = sys.argv[3]  #hwcmpvcp1
 fileout =sys.argv[4]
