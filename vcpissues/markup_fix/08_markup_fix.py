"""
Markup fixer + audit for vcp.txt (VCP).
Counterpart of PWG#175, PWK#113, and siblings.

VCP-specific notes: No paired tags. Completely clean.

Inputs: ../../../csl-orig/v02/vcp/vcp.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "vcp" / "vcp.txt", HERE / "vcp.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "vcp_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(NONE)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for vcp.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"VCP markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against vcp.txt.\n")
        f.write(f"Paired tags: none\n")
        f.write(f"Notes: No paired tags. Completely clean.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in none\n\n")
