"""hwcmpa.py Feb 19, 2014
 Modified from hwcmp.py 
 Compare vachw1.txt and vcphw1.txt 
 Does NOT coalesce consecutive duplicate headwords; this is because
 neither vac.txt nor vcp0.txt coalesces.  The coalescing was just for
 vcpte.
"""
...
def extract2_recs(filein):
 """ This function modified regarding dups.
...
def hw_cmp7(filein1,filein2,fileforce,fileout):
 ...
 while (i1<nrecs1) and (i2<nrecs2):
  ...
  if rec2.ids in force2map:
   ...
   i2 = i2 + 1
   continue
  if False and rec2.hw == recs2[i2-1].hw: # skip this for vac-vcp
   (rec1ids,eqtype,rec2ids) = recs[-1]
   rec2ids = "%s;%s" % (rec2ids,rec2.ids)
   recs[-1] = (rec1ids,eqtype,rec2ids)
   i2 = i2 + 1
   continue
  if False and rec1.hw == recs1[i1-1].hw: # skip this for vac-vcp
   (rec1ids,eqtype,rec2ids) = recs[-1]
